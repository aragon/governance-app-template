...
const VetoTally: FC<VoteTallyProps> = ({ voteCount, votePercentage }) => (
  <Card>
    <div className="flex flex-row space-between pb-2">
      <p className={`flex-grow text-xl text-primary-500 font-semibold`}>Vetoed</p>
      <p className="text-xl font-semibold">{compactNumber(formatUnits(voteCount + BigInt(1) || BigInt(0), 18))}</p>
    </div>
    <div className={`h-4 w-full bg-primary-100 rounded`}>
      <div className={`h-4 bg-primary-700 rounded`} style={{ width: `${Math.min(votePercentage, 100)}%` }} />
    </div>
  </Card>
);
